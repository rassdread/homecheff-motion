#!/usr/bin/env npx tsx
/**
 * PX.4A.6.3 Production visual cert — untracked helper.
...
async function globalSnapshot(page: Page) {
  ...
  return {
    ...
    canvases: await page.locator("[data-testid='px4a-preview-canvas']").count(),
  };
}
...
async function main() {
  ...
  try {
    ...
    const d = report.desktop as { overflow: number; style: string; duration: string; canvases: number; more: boolean; musicPanel: number; finish: number };
