#!/usr/bin/env npx tsx
/**
 * PX.4A.6.4 Production attach regression — untracked helper.
 * HomeCheff sell/new → 4 listing photos → from-item → 15s → hc_shards → Video gebruiken.
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (await titleBox.count()) await titleBox.fill("PX.4A.6.4 attach — niet publiceren");
    else await page.locator("input[required]").first().fill("PX.4A.6.4 attach — niet publiceren");
    const desc = page.getByLabel(/Omschrijving|Vertel wat je aanbiedt/);
    if (await desc.count()) await desc.first().fill("PX.4A.6.4 disposable listing. Do not publish.");
    ...
    await page.locator('[data-testid="px4a-transition-hc_shards"]').click();
    result.transition = "hc_shards";
