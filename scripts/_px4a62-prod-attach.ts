#!/usr/bin/env npx tsx
/**
 * PX.4A.6.2 Production attach regression — untracked helper.
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (await titleBox.count()) await titleBox.fill("PX.4A.6.2 attach — niet publiceren");
    else await page.locator("input[required]").first().fill("PX.4A.6.2 attach — niet publiceren");
    const desc = page.getByLabel(/Omschrijving|Vertel wat je aanbiedt/);
    if (await desc.count()) await desc.first().fill("PX.4A.6.2 disposable listing. Do not publish.");
