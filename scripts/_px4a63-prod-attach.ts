#!/usr/bin/env npx tsx
/**
 * PX.4A.6.3 Production attach regression — untracked helper.
 * HomeCheff sell/new → 4 listing photos → from-item → selected-photo edit → 15s → Video gebruiken.
...
const OUT_DIR = join(REPO_ROOT, "docs/audits/px4a63-prod-cert");
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (await titleBox.count()) await titleBox.fill("PX.4A.6.3 attach — niet publiceren");
    else await page.locator("input[required]").first().fill("PX.4A.6.3 attach — niet publiceren");
    const desc = page.getByLabel(/Omschrijving|Vertel wat je aanbiedt/);
    if (await desc.count()) await desc.first().fill("PX.4A.6.3 disposable listing. Do not publish.");
