...
export type StartExternalMergeJobInput = {
  projectId: string;
  /** Prisma `AnimationExport.id` — worker callbacks and stale-job checks use this. */
  exportId: string;
  /** Correlates worker job, DB `providerJobId`, and callbacks (generated by the app). */
  jobId: string;
  /** When set (production), worker POSTs merge completion to this Motion app URL. */
  callbackUrl?: string;
...
export async function startExternalMergeJob(
  ...
  options?: { logRetryPhases?: boolean }
  ...
  const logRailway = (phase: string, extra: Record<string, unknown>) => {
    if (options?.logRetryPhases) {
      hcExportRetryLog("server", phase, {
        projectId: input.projectId,
        exportId: input.exportId,
        jobId: input.jobId,
        ...extra,
  ...
  logRailway(options?.logRetryPhases ? "export_retry.railway_request_start" : "export_merge.railway_request_start", {
    url,
    fragmentCount: input.transitionVideos.length,
  });
  ...
  try {
    ...
    if (options?.logRetryPhases) {
      hcExportRetryLog("server", "export_retry.failed", {
        projectId: input.projectId,
        exportId: input.exportId,
        jobId: input.jobId,
        message: msg,
  ...
  logRailway(options?.logRetryPhases ? "export_retry.railway_response" : "export_merge.railway_response", {
