  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("error", (err: NodeJS.ErrnoException) => {
      if (err.code === "ENOENT") {
        reject(
          new Error(
            `FFmpeg executable not found (${binary}). Install ffmpeg or set FFMPEG_PATH.`
          )
        );
        return;
      }
...
export async function startProjectExport(projectId: string) {
  return runExclusiveExport(projectId, () => runProjectExportMerge(projectId));
}
...
async function runProjectExportMerge(projectId: string) {
  const project = await getAnimationProjectById(projectId);
  if (!project) {
    throw new Error("Project not found.");
  }
  ...
  if (latestExport?.status === "completed" && latestExport.outputVideoUrl) {
    ...
    return getAnimationProjectById(projectId);
  }
