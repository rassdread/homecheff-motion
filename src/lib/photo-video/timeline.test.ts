...
describe("PX.4A.1 timeline + local motion", () => {
  it("builds overlapping clips that match the duration formula", () => {
    const c = addPhotos(
    ...
    assert.equal(clips[1]?.startSeconds, 2 - 0.4);
    const head = playheadAt(c, 1.8);
    assert.ok(head.from);
    assert.ok(head.to);
  ...
  it("motion is deterministic", () => {
    ...
    assert.deepEqual(sampleLocalMotion("pan", 0.3, 0.1), sampleLocalMotion("pan", 0.3, 0.1));
  });
});
