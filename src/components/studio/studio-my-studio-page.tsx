...
export function StudioMyStudioPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!res.ok) {
        queueMicrotask(() => setError(t("studio.myStudio.error.loadFailed")));
        return;
      }
  ...
  return (
    <StudioAuthGate>
      <main className={`flex min-h-screen flex-col ${brand.softGradientBg}`}>
        <StudioShellHeader projectTitle={t("studio.myStudio.title")} />

        <section className="mx-auto w-full max-w-4xl flex-1 px-4 py-8 sm:px-6">
          {error ?
            <p className="text-sm text-red-700">{error}</p>
          : !report ?
            <p className="text-sm text-zinc-600">{t("studio.myStudio.loading")}</p>
          : (
            <div className="space-y-8">
              <header>
                <p className="text-xs font-semibold uppercase tracking-widest text-[#006D52]">
                  {t("studio.myStudio.eyebrow")}
                </p>
                <h2 className="mt-1 text-2xl font-bold text-zinc-900">{t("studio.myStudio.title")}</h2>
                <p className="mt-2 text-sm text-zinc-600">{t("studio.myStudio.subtitle")}</p>
              </header>

              <div>
                <h3 className="text-sm font-semibold text-zinc-900">{t("studio.myStudio.usageThisMonth")}</h3>
              ...
              <div>
                <h3 className="text-sm font-semibold text-zinc-900">{t("studio.myStudio.recentActivity")}</h3>
                ...
                : (
                  <ul className="mt-3 divide-y divide-zinc-100 rounded-2xl border border-zinc-200 bg-white">
                    {report.recentActivity.map((item) => (
                      <li key={item.id} className="flex min-h-[44px] items-center justify-between gap-3 px-4 py-3">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Link
                  href="/studio"
                  className="inline-flex min-h-[44px] items-center rounded-full bg-[#006D52] px-5 text-sm font-semibold text-white hover:bg-[#005a44]"
                >
                  {t("studio.myStudio.backToStudio")}
                </Link>
              </div>
            </div>
          )}
