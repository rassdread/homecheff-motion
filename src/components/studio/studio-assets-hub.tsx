...
function HubGroupBlock({
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-6">
      ...
      <p className="mt-1 text-sm text-slate-600">{t(`studio.assetsHub.groupDesc.${group}` as never)}</p>
        ...
        {sections.map((section) => {
          ...
          return (
            <Link
              key={section.section}
              href={section.href}
              className="flex min-h-[120px] flex-col rounded-2xl border border-slate-100 bg-slate-50/50 p-4 transition-colors hover:border-[#006D52]/30 hover:bg-[#006D52]/5"
            >
              <span className="text-sm font-semibold text-slate-900">{t(section.labelKey as never)}</span>
              <span className="mt-1 flex-1 text-xs text-slate-600">{t(section.descriptionKey as never)}</span>
