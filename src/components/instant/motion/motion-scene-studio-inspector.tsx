...
export function MotionSceneStudioInspector({ context, storyboardTitle }: Props) {
  ...
  const textBeats = context.studioTextBeats;
  const [textBeatsOpen, setTextBeatsOpen] = useState(true);
      ...
      {textBeats ?
        <div className="rounded-xl border border-sky-200/80 bg-sky-50/60 p-3">
          <button
            type="button"
            className="flex w-full items-center justify-between text-left"
            onClick={() => setTextBeatsOpen((v) => !v)}
            aria-expanded={textBeatsOpen}
          >
            <p className="text-xs font-semibold uppercase tracking-wide text-sky-900">
              {t("motion.handoff.textBeats.title")}
            </p>
            <span className="text-xs text-zinc-500">{textBeatsOpen ? "▲" : "▼"}</span>
          </button>
          {textBeatsOpen ?
            <div className="mt-2 space-y-2 text-xs text-zinc-800">
              <p className="text-[10px] text-sky-800">
                {t("motion.handoff.textBeats.source")}:{" "}
                {textBeats.source === "studio_auto"
                  ? t("motion.handoff.textBeats.autoGenerated")
                  : textBeats.source}
              </p>
              {textBeats.headlineBeats[0] ?
                <p>
                  <span className="font-semibold">{t("motion.handoff.textBeats.headline")}:</span>{" "}
                  {textBeats.headlineBeats[0]}
                </p>
              : null}
              {textBeats.subtitleBeats[0] ?
                <p>
                  <span className="font-semibold">{t("motion.handoff.textBeats.subheadline")}:</span>{" "}
                  {textBeats.subtitleBeats[0]}
                </p>
              : null}
              {textBeats.heroText ?
                <p>
                  <span className="font-semibold">{t("motion.handoff.textBeats.hero")}:</span>{" "}
                  {textBeats.heroText}
                </p>
              : null}
              {textBeats.heroFinaleText ?
                <p>
                  <span className="font-semibold">{t("motion.handoff.textBeats.finale")}:</span>{" "}
                  {textBeats.heroFinaleText}
                </p>
              : null}
              {textBeats.beatLines.length > 0 ?
                <p>
                  <span className="font-semibold">{t("motion.handoff.textBeats.beatLines")}:</span>{" "}
                  {textBeats.beatLines.join(" · ")}
                </p>
              : null}
              {textBeats.usedFields.length > 0 ?
                <p className="text-[10px] text-sky-800">
                  {t("motion.handoff.textBeats.used")}: {textBeats.usedFields.join(", ")}
                </p>
              : null}
              {textBeats.ignoredFields.length > 0 ?
                <p className="text-[10px] text-zinc-500">
                  {t("motion.handoff.textBeats.ignored")}: {textBeats.ignoredFields.join(", ")}
                </p>
              : null}
            </div>
          : null}
